...
from ui.styles import (CUSTOM_CSS, get_model_badge_html, get_social_selector_html,
                       get_selection_cards_html, SOCIAL_MEDIA_DEFS, SOCIAL_ICONS)
...
with gr.Blocks(title="ChismesitoGPT v2", css="""
...
<div style="text-align: center; margin-bottom: 24px;">
  ...
  <p style="margin: 0; font-size: 12px; color: rgba(255,255,255,0.4);
    ...
    with gr.Column(visible=False, elem_id="app-panel") as app_panel:
        ...
        # ── Fila principal: Prompt + Calculadora de costos ──
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=3, min_width=260):
                cost_display = gr.Markdown(
                    "💰 **Costos de sesión**\nEjecuta una búsqueda para ver el detalle.",
                    elem_classes=["status-md"],
                    label="Calculadora de costos"
                )

        # ── Info del modelo activo (banner informativo) ──
        ...
        # Inyectamos un <input type="hidden"> DENTRO del HTML de los chips
        social_html += '<input type="hidden" id="social-hidden-real" value="youtube" />'
        social_selector_html = gr.HTML(value=social_html)
        _social_dummy = gr.Textbox(value="unused", visible=False)  # placeholder para inputs
        ...
        with gr.Accordion("📍 Configuración de Google Maps (Búsqueda Geográfica)", open=False):
            ...
            # Panel geográfico — componentes NATIVOS de Gradio (sin JS frágil).
        ...
        # ── Modo de extracción: elegir manualmente (default) vs automático ───
        ...
        with gr.Column(visible=False) as selection_panel:
            gr.Markdown("### 🎯 Elige de qué contenido extraer comentarios")
        ...
        # Textbox oculto: el JS de fetch_btn inyecta aquí el JSON de selección.
        _selection_dummy = gr.Textbox(value="{}", visible=False)

        # ── Chat RAG ─────────────────────────────────────────────────────────
        gr.Markdown("---\n### 💬 Chat con los datos")
        # ── Banner modelo + selector inline ─────────────────────────────────
    ...
    # Model change — actualiza badge + banner superior + model_status
    def _handle_model_change_full(model, state):
        badge  = _build_badge(model)
        info   = next((m for m in AVAILABLE_MODELS if m["id"] == model), None)
        p      = info["pricing"] if info else ""
        state.model = model
        return badge, f"`{model}` — {p}" if p else f"`{model}`", _build_model_banner(model), state

    model_selector.change(
        fn=_handle_model_change_full,
        inputs=[model_selector, session_state],
        outputs=[model_badge, model_status, model_info_banner, session_state]
    )
...
if __name__ == "__main__":
    ...
    # Matar proceso previo en el puerto (evita error 10048)
    ...
    demo.launch(
        ...
        css=CUSTOM_CSS,
        js=SOCIAL_JS,     # ← Gradio 6.0: js va en launch(), no en Blocks()
    )
