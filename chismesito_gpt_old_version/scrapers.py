...
####### FACEBOOK ##############
#### Función para obtener de un  link de facebook 
def get_facebook_id(link):
    """
    Extrae el ID del grupo o del post de un enlace de Facebook.

    Args:
        link (str): El enlace de Facebook.

    Returns:
        tuple: Una tupla que contiene el tipo de ID ('group' or 'post')
               y el ID extraído, o (None, None) if no valid ID is found.
    
    """
    group_regex = re.compile(r'/groups/(\d+)/?')
    post_regex = re.compile(r'/posts/(pfbid[a-zA-Z0-9]+)')
    if '/groups/' in link: 
        match = group_regex.search(link)
        if match:
            print(f"Group ID: {match.group(1)}")
            return 'group', match.group(1)
    elif '/posts/' in link: 
        match = post_regex.search(link)
        if match:
            print(f"Group ID: {match.group(1)}")
            return 'post', match.group(1)
    return None, None 

def search_fbposts_rapidapi(query, api_key):
    query = str(query)
    try:
        ...
        print(f'Error al descargar comentarios de post con RapidAPI: {e}')
        return None 
    
def download_fbcomments_rapidapi_method1(url, api_key):
    url = str(url)
    id_type, post_id = get_facebook_id(url)
    if id_type !='post' or not post_id:
        print(f'No se pudo extraer un ID de post válido de la URL:{url}')
        return pd.DataFrame()
    try:
        ...
        df = df[['author', 'comment_id', 'message']]
        df.rename(columns={'message': 'comment'}, inplace=True)
        return df
    except requests.exceptions.RequestException as e:
        print(f"Error al descargar comentarios de post con RapidAPI método 1: {e}")
        return pd.DataFrame()
    except Exception as e:
        print(f"Error inesperado al descargar comentarios de post con RapidAPI método 1: {e}")
        return pd.DataFrame()
    
def download_fbcomments_rapidapi_method2(query, api_key):
    query = str(query)
    try:
        url = "https://facebook-pages-scraper2.p.rapidapi.com/get_facebook_post_comments_details"
        ...
        if 'data' in response and 'comments' in response['data']:
             df = pd.DataFrame(response['data']['comments'])
             # Check if necessary columns exist before renaming
             if 'author' in df.columns and 'comment_id' in df.columns and 'comment_text' in df.columns:
                df = df[['author', 'comment_id', 'comment_text']]
                df.rename(columns={'comment_text': 'comment'}, inplace=True)
                return df
             else:
                print("Columnas esperadas no encontradas en la respuesta de RapidAPI método 2.")
                return pd.DataFrame()
        else:
            print("Estructura de respuesta inesperada de RapidAPI método 2.")
            return pd.DataFrame()
    except requests.exceptions.RequestException as e:
        print(f'Error al descargar comentarios de post con RapidAPI método 2: {e}')
        return pd.DataFrame()
    except Exception as e:
        print(f'Error inesperado al descargar comentarios de post con RapidAPI método 2: {e}')
        return pd.DataFrame()

def get_fb_comments_robust(url, api_key):
    """
        Intenta desccargar comentarios de FB usando la API  de RapidApi.
        Regresa un dataframe vacío si los métodos fallan
        Args: 
            url(cadena): El URL de Facebook
            api_key(cadena): La API Key de RapidApi, habilitada para usar el método
    """
    print(f"Intentando descargar comentarios para: {url} con RapidAPI método 2")
    df = download_fbcomments_rapidapi_method2(url, api_key)

    if df.empty:
        print(f"RapidAPI método 2 falló o devolvió un DataFrame vacío. Intentando con RapidAPI método 1.")
        df = download_fbcomments_rapidapi_method1(url, api_key)

    if df.empty:
        print(f"Ambos métodos de RapidAPI fallaron para la URL: {url}. Devolviendo DataFrame vacío.")
    return df

####### INSTAGRAM ##############
def get_instagram_post_id(url):
    """
    Extrae el post ID de un enlace de Instagram.

    Args:
        url (str): El enlace de Instagram.

    Returns:
        str: El post ID si se encuentra, de lo contrario None.
    """
    # Regex para encontrar el ID en el formato /p/{id}/ o /reel/{id}/
...
def download_instagramcomments_rapidapi(query, api_key):
    query = str(query)
    instagram_id = get_instagram_post_id(query)
    try:
        url = "https://instagram-social-api.p.rapidapi.com/v1/comments"

        querystring = {"code_or_id_or_url":instagram_id}
        ...
        df = df[['user_id', 'id', 'text']]
        df.rename(columns={'text': 'comment', 'user_id': 'author', 'id': 'comment_id'}, inplace=True)
        return df
    except Exception as e:
        print(f'Error al descargar comentarios de post con RapidAPI: {e}')
        return None
